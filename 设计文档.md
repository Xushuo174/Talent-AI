...
设计基线固定为 **n8n 2.38.7**，发布标签 `n8n@2.38.7`，本地源码基线提交 `fa34d4cd`。该版本根目录配置要求 Node.js ≥24、pnpm ≥11.22，开发统一使用 Node.js 24.x 与 pnpm 11.22.0，并保留上游锁文件。版本与环境依据来自[发布记录](https://github.com/n8n-io/n8n/releases/tag/n8n%402.38.7)和本地固定源码。
...
11. **进入进阶版本**：再处理 Codex、自举、长期状态、Skill、复杂并行和增强界面。
...
第一版状态只保证单次 Workflow Execution 内有效。跨重启恢复、人工暂停继续和 Codex thread 恢复保留到进阶版本。未来 Codex Coding Agent 应让每次 Workflow Execution 创建一个 thread，同次执行的多轮共享该 thread，并继续把测试与评审转换为 `LoopEvaluationV1`，由 Goal Loop 统一裁决。
